$(document).ready(function(){
		...
		$.ajax({
			...
			success:function(data){
				...
				for(i = 0; i < data.length; i++){
					html += '<div class="row" style="padding-top:40px;border-bottom:1px solid #dedede">'+
			          '<div class="col-lg-12">'+
			            '<small style="float:right;margin-top:-30px;font-size:11px">Date: '+data[i].datetimepublished+'</small>'+
			            '<b><font style="font-size:16px;color:black">'+data[i].title+'</font></b><br>';
			            var xb = Number(data[i].aut.length);
			              html+='<font style="font-size:15px">';
			            ...
			            html+='<div style="padding-top:10px;text-align:justify;">'+
			              '<font style="float:right;margin-top:-40px;color:#FFAD99">New</font>'+
			              '<b><font style="font-size:16px">'+showMore(data[i].abstract)+'...</font></b>'+
			            '</div>'+
			            '<div style="padding-top:20px;text-align:justify;">'+
			              '<b><font style="font-size:16px"></font></b>'+
			            '</div>'+
			          '</div>'+
			        '</div>';
				}
			...
			success:function(data){
				...
				for(i = 0; i < max; i++){
					...
					html += '<tr style="" class="POData">'+
                        ...
                        html+='<td style="padding:3px;" align="center">'+data[i].vol_page+'</td>';
                        var xb = Number(data[i].aut.length);
	...
	$('#pagination').on('click','.nextT',function(){
		...
		$.ajax({
			...
			success:function(data){
				var html = '', page = '';
				var i, a = oldsetpage, num = 0, mod = 0, max = 7, l = 0;
				pageTootal = Number(data.length);
				if(pageTootal < 7){
					max = Number(data.length);
				}
	...
	$('#pagination').on('click','.prevV',function(){
		...
		if(updateset > 7){
			...
			$.ajax({
				...
				success:function(data){
					var html = '', page = '';
					var i, a = oldsetpage, num = 0, mod = 0, l = 0;
					pageTootal = Number(data.length);
					if(pageTootal < 7){
						max = Number(data.length);
					}
